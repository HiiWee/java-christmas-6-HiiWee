...
- 사용자는 12월달 1~31일 사이에 예약할 날짜를 고르고, 예약할 메뉴를 고릅니다.
- 우테코 이벤트 플래너에선 해당 날짜와 메뉴에 맞는 할인 및 증정 혜택을 제공해줍니다!!

### 주의사항

- 10_000원 이하의 예약은 이벤트에 참여할 수 없습니다!
- 음료만 포함된 주문을 예약할 수 없습니다!
- 증정 이벤트는 할인 금액에 포함되지 않습니다!
...
- [x] 각 메뉴가 존재하고 메뉴에 대한 타입이 각 메뉴를 포함하고 있습니다.
- [x] 메뉴 목록이 존재합니다.
- [x] 입력하는 메뉴는 `해산물파스타-2,레드와인-1,초코케이크-1` 와 같이 입력한다.
    - [x] `검증` 음료만 주문하면 주문할 수 때없습니다.
    - [x] 아래 예외 메시지는 `"[ERROR] 유효하지 않은 주문입니다. 다시 입력해 주세요."` 공통
    - [x] `검증` 없는 메뉴를 입력하면 예외
...
- [x] 이벤트 타입은 크리스마스 디데이 할인, 평일 할인, 주말 할인, 특별 할인, 증정 이벤트가 존재합니다.
    - [x] 전체 이벤트에 5개의 이벤트가 존재하고, 할인 이벤트와 증정 이벤트로 분류가 나뉩니다.
...
- [x] 이벤트에 참여했다면 참여한 이벤트와, 이벤트 참여후에 얻은 금액을 저장해야 합니다.
    - 증정 이벤트
        - [x] 할인 전 총 주문 금액이 120_000원 이상일때 샴페인(25_000원)을 1개 증정합니다.

이벤트 적용 로직

- [x] 입력한 날짜와 메뉴에 따른 이벤트를 전부 적용한다.
    - [x] 이벤트에 참여할 수 있는지의 여부를 확인한다.
    - [x] 이벤트에 참여할 수 있다면 모든 이벤트를 참여한다.
    - [x] 적용되는 혜택이 0원이라면 제외한다.
- [x] 이벤트 적용 이후 배지를 부여한다.

### 이벤트 배지

- [x] 위의 할인 이벤트 혜택 금액에 다른 배지를 부여한다.
    - `별`: 5000원 이상
    - `트리`: 10_000원 이상
    - `산타`: 20_000원 이상
...
- [x] 입력한 메뉴와, 할인전 총 금액을 출력한다.
- [x] 증정 메뉴를 출력한다.
- [x] 혜택 내역 및 금액을 출력한다.
    - [x] 총혜택 금액 = 할인 금액의 합계 + 증정 메뉴의 가격
- [x] 할인 후 예상 지출 금액을 출력한다.
    - [x] 할인 후 예상 결제 금액 = 할인 전 총주문 금액 - 할인 금액(증정 메뉴는 제외)
- [x] 배지를 출력한다.
